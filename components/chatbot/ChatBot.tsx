...
const BUILT_IN_FAQS: Faq[] = [
  ...
  {
    id: '__bi_attendance_rate',
    question: '내 출석률은 어디서 확인하나요?',
    answer: '📅 **출석률 확인 방법**\n\n**개인 출석률**\n1. 사이드바에서 **"출석"** 메뉴를 클릭하세요.\n2. 페이지 상단에 **전체 출석률 %** 와 세션별 세부 현황이 표시됩니다.\n3. 각 세션 카드를 클릭하면 입실·퇴실 시간과 해당 세션 출석률을 볼 수 있어요.\n\n**출석률 계산 방식**\n실제 체류 시간 ÷ 세션 진행 시간 × 100\n(80% 이상: 초록, 1~79%: 주황, 0%: 회색)',
    keywords: ['출석률', '출석', '출결', '출석확인', '출석현황', '입실', '퇴실', '출석%'],
  },
  {
    id: '__bi_notice',
    question: '공지사항은 어디서 확인하나요?',
    answer: '📢 **공지사항 확인 방법**\n\n1. 사이드바에서 **"커뮤니티"** 메뉴를 클릭하세요.\n2. 상단에 **📌 고정된 공지글**이 있으면 가장 먼저 표시됩니다.\n3. 운영진이 올린 공지를 확인하세요.\n\n💡 새 공지가 올라오면 사이드바 **🔔 알림 벨**에 알림이 표시됩니다. 벨 아이콘을 클릭해 확인해 보세요!',
    keywords: ['공지', '공지사항', '안내', '알림', '소식', 'notice', '커뮤니티'],
  },
  {
    id: '__bi_9',
            ...
            <div className="grid grid-cols-3 gap-1.5 auto-rows-fr">
              {QUICK_QUESTIONS.map((q) => (
                <button
                  ...
                  className="text-xs bg-indigo-50 border border-indigo-100 text-indigo-600 hover:bg-indigo-100 rounded-lg px-2 py-2 transition-colors text-center leading-tight min-h-[44px] flex items-center justify-center"
                >
                  {q}
                </button>
              ))}
